...
# Modified from DynamiCrafter, https://github.com/Doubiiu/DynamiCrafter/tree/main/scripts/evaluation/inference.py
...
def generate_shots(args, gpu_num, gpu_no, short_desc_path):
    ...
    with torch.no_grad(), torch.cuda.amp.autocast():
        for idx, indice in tqdm(enumerate(range(0, len(prompt_list_rank), args.bs)), desc='Sample Batch'):
            # Adjacent Latent Transition Mechanisms
            if idx == 0:  # The first shot
                cond_z0 = torch.randn(noise_shape)
            else:
                # Calculate the standard deviation of boundary noise
                gamma = 1.0  # The noise scaling factor, can be adjusted as a hyperparameter
                alpha = 0.5  # The parameter controlling time continuity, range [0,1]
                beta_i = gamma * (1 - idx / num_samples)  # Adjust the noise amplitude based on the current shot index
                
                # Get the final latent frame of the previous shot
                prev_shot_final_latent = batch_latents_list[-1]  # Assume batch_latents_list stores the generation result of the previous shot
                
                # Generate boundary noise
                boundary_noise = torch.randn(noise_shape) * math.sqrt(beta_i)
                
                # z_{i+1}^0 = ε_boundary + α · z_i^f
                cond_z0 = boundary_noise + alpha * prev_shot_final_latent
...
def main():
    ...
    print(f"Avatar generation completed successfully!, saved to {args.avatar_json_path}")
