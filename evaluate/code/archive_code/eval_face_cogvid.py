...
# Set up logging
logging.basicConfig(filename="./result/cogvid_video_similarity_log.txt", level=logging.INFO, format="%(message)s")
...
def evaluate_all_videos_in_directory(directory):
    ...
    for i in range(1, 31):  # Assuming videos are named sequentially from 1 to 30
        video_path = os.path.join(directory, f"video_{i:02d}.mp4")  # Format file names as video_01.mp4, video_02.mp4, etc.
    ...
    with open("./result/cogvid_similarities.csv", mode="w", newline="") as csv_file:
...
directory = "/storage/home/mingzhe/code/CogVideo/inference/result_t2v_v2"
