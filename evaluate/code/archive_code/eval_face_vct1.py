...
# Set up logging
logging.basicConfig(filename="./result/vct1_video_similarity_log.txt", level=logging.INFO, format="%(message)s")
...
def evaluate_all_videos_in_directory(directory):
    ...
    for i in range(1, 31):  # Assuming videos are named sequentially from 1 to 30
        video_path = os.path.join(directory, f"{i:04d}.mp4")  # Format file names as 0001.mp4, 0002.mp4, etc.
    ...
    with open("./result/vct1_similarities.csv", mode="w", newline="") as csv_file:
...
directory = "/storage/home/mingzhe/code/VideoCrafter/results_t2v_v1/base_512_v1"
