...
# Set up logging
logging.basicConfig(filename="./result/video_similarity_log.txt", level=logging.INFO, format="%(message)s")
...
def evaluate_all_videos_in_directory(directory):
    ...
    for i in range(1, 31):  # Assuming videos are named sequentially from 1 to 30
        video_path = os.path.join(directory, f"sample_ip_{i}_sample0.mp4")
    ...
    with open("./result/vgot_similarities.csv", mode="w", newline="") as csv_file:
...
directory = "/storage/home/mingzhe/code/VideoGen-of-Thought/thirdparty/i2v-module-3/DynamiCrafter/results/VGoT/dynamicrafter_vgot_mary_life2_1024_mp_seed123/samples_separate"
